...
const SECONDS = +secondsArg, NEXT_AT = +nextAtArg; const DRIVE = process.env.DRIVE || "on"; const SHOT_MS = +(process.env.SHOT_MS || 700); const NEXT2 = +(process.env.NEXT2 || 0);
mkdirSync(outdir, { recursive: true });
const port = 9333 + Math.floor(Math.random() * 100);
const chrome = spawn("/Applications/Google Chrome.app/Contents/MacOS/Google Chrome", [
  "--headless=new", `--remote-debugging-port=${port}`, `--user-data-dir=${outdir}/profile`, "--window-size=1100,720",
  "--use-angle=swiftshader", "--enable-unsafe-swiftshader", "--ignore-gpu-blocklist", "--no-first-run", "--hide-scrollbars", "--mute-audio",
  "about:blank",
], { stdio: "ignore" });
const done = async (code = 0) => { try { chrome.kill("SIGKILL"); } catch {} process.exit(code); };
process.on("SIGINT", () => done(1));

let ws; let idc = 0; const pending = new Map(); const events = [];
...
try {
  ...
  await evaluate(`new Promise(r => { const m = window.__m; if (m.loaded()) return r(1); m.once('idle', () => r(2)); setTimeout(() => r(0), 20000); })`);
  ...
  const T0 = Date.now(); let nextDone = false, next2Done = false; let k = 0;
  while ((Date.now() - T0) / 1000 < SECONDS) {
    const t = (Date.now() - T0) / 1000;
    if (NEXT2 && !next2Done && t >= NEXT2) { await evaluate(`window.__parkwildStore.getState().tourNext(); 1`); next2Done = true; console.log("next2 at", t.toFixed(1)); }
    if (!nextDone && t >= NEXT_AT) { await evaluate(`window.__parkwildStore.getState().tourNext(); 1`); nextDone = true; console.log("next at", t.toFixed(1)); }
    await shot(`f${String(k++).padStart(2, "0")}_${t.toFixed(1)}s`);
    await sleep(SHOT_MS);
  }
  const samples = await evaluate(`JSON.stringify({ frames: window.__frames, vis: document.visibilityState, samples: window.__samples, mapErrors: window.__err, style: (() => { const st = window.__m.getStyle(); return st.layers.filter(l => /mask|imagery|hillshade|outline/.test(l.id)).map(l => ({ id: l.id, type: l.type, layout: l.layout, paint: l.paint })); })(), maxB: window.__m.getMaxBounds() && window.__m.getMaxBounds().toArray() })`);
  writeFileSync(`${outdir}/samples.json`, samples);
  const s = JSON.parse(samples);
  console.log("frames rendered:", s.frames, "visibility:", s.vis, "samples:", s.samples.length, "mapErrors:", JSON.stringify(s.mapErrors).slice(0, 400));
  ...
  console.log("errors:", errs.length); for (const e of errs.slice(0, 8)) console.log("  ", JSON.stringify(e.params).slice(0, 300));
  await done(0);
} catch (e) { console.error("probe failed:", e.message); await done(1); }
