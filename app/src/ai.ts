...
import type { Landmark, Species } from "./types";
...
function modelOverride(): string | null { try { return localStorage.getItem("parkwild:ai-model"); } catch { return null; } }
...
export function engineReady(): boolean { return engine !== null; }

export function loadEngine(onProgress: (text: string, pct: number) => void): Promise<MLCEngine> {
  if (engine) return Promise.resolve(engine);
  if (!loading) {
    loading = import("@mlc-ai/web-llm").then(async (webllm) => {
      const opts = { initProgressCallback: (r: { text: string; progress: number }) => onProgress(r.text, r.progress) };
      ...
      for (const cacheBackend of ["cache", "opfs", "indexeddb"] as const) {
        try {
          e = await webllm.CreateMLCEngine(MODEL_ID, { ...opts, appConfig: { ...webllm.prebuiltAppConfig, cacheBackend } });
          break;
        } catch (err) {
          lastErr = err;
          if (!/quota/i.test(String(err))) throw err;
          onProgress(`${cacheBackend === "cache" ? "Cache storage" : cacheBackend === "opfs" ? "The private file system" : "IndexedDB"} refused the download; trying another store…`, 0);
        }
      }
      if (!e) throw lastErr;
      engine = e;
...
export interface Fact { n: number; text: string; href?: string; label?: string; }
export interface Grounding { facts: Fact[]; species: Species[]; places: Landmark[]; intents: string[]; month: number | null; }
...
const GENERIC = new Set(["american", "common", "northern", "western", "eastern", "southern", "great", "greater", "lesser", "little",
  "mountain", "rocky", "north", "canada", "canadian", "grey", "gray", "red", "white", "black", "brown", "yellow", "blue", "golden",
  "ground", "tree", "rock", "sage", "spotted", "striped", "long", "short", "tailed", "eared", "headed", "billed", "winged", "the", "of", "and"]);

function singular(w: string): string {
  if (w === "wolves") return "wolf"; if (w === "geese") return "goose"; if (w === "mice") return "mouse";
...
export function matchSpecies(q: string, all: Species[]): Species[] {
  ...
  for (const s of all) {
    ...
    for (const n of [s.common_name ?? "", ...(s.other_names ?? []), s.scientific_name]) {
      const nn = norm(n);
      if (!nn) continue;
      if (q.includes(nn) || q.includes(nn + "s")) { best = Math.max(best, 3); continue; }
      const words = nn.split(" ");
      const distinctive = words.filter((w) => !GENERIC.has(w));
      if (distinctive.length && distinctive.every((w) => qWords.has(w))) best = Math.max(best, distinctive.length === words.length ? 2.5 : 2);
    }
  ...
  return scored.sort((a, b) => b.score - a.score || b.s.sightings - a.s.sightings).slice(0, 3).map((x) => x.s);
}
...
export function ground(question: string): Grounding {
  ...
  const facts: Fact[] = [];
  const add = (text: string, href?: string, label?: string) => { if (facts.length < MAX_FACTS) facts.push({ n: facts.length + 1, text, href, label }); };
  const intents: string[] = [];
  for (const [k, re] of Object.entries({
    plan: /\b(plan|itinerary|day|hours?|route|trip|visit)\b/, camp: /\b(camp|campground|campsite|tent|rv|sleep|stay|lodge|hotel|cabin)\b/,
    hike: /\b(hike|hiking|trail|walk|walking)\b/, when: /\b(when|month|season|time of year|best time)\b/, where: /\b(where|see|find|spot|spots|most|likely)\b/,
    model: /\b(model|camera|ai|computer vision|detection|detect)\b/, photo: /\b(photo|picture|image)\b/, count: /\b(how many|number|count|total)\b/,
  ...
  const places = landmarks.filter((l) => { const nn = norm(l.name); return nn && q.includes(nn); }).slice(0, 3);
  ...
  for (const s of all) s.months.forEach((m, i) => { effort[i] += m; });
  const eTotal = effort.reduce((a, b) => a + b, 0) || 1;
  add(`${st.parkName}: ${total.toLocaleString()} recorded sightings of ${all.length} species from iNaturalist research-grade observations and GBIF datasets; ${Math.round((100 * (effort[5] + effort[6] + effort[7])) / eTotal)}% of all sightings fall in June to August because that is when people visit.`, undefined, "Park summary");
  if (stops.length) add(`The park's tour has ${stops.length} stops in order: ${stops.map((s) => s.name).join(", ")}.`, undefined, "Tour");
  ...
  if (cp) {
    ...
    add(ran.length
      ...
      : `The roadside camera pass has not run in ${st.parkName} yet; every count on the site is from people.`, undefined, "Camera pass");
  }
  ...
  for (const s of species) {
    ...
    if (cells && !s.suppression) {
      ...
      const top = cells.features.filter((f) => !f.properties.coarsened).map((f) => ({ f, e: f.properties.sp.find((x) => x[0] === idx) }))
        .filter((x): x is { f: typeof x.f; e: NonNullable<typeof x.e> } => !!x.e).sort((a, b) => b.e[1] - a.e[1]).slice(0, 3);
      const named = top.map(({ f, e }) => {
        const ring = f.geometry.coordinates[0]; let cx = 0, cy = 0; for (let i = 0; i < ring.length - 1; i++) { cx += ring[i][0]; cy += ring[i][1]; } cx /= ring.length - 1; cy /= ring.length - 1;
        let best: Landmark | null = null, bd = Infinity;
        for (const l of landmarks) { const d = haversineM(cx, cy, l.lon, l.lat); if (d < bd) { bd = d; best = l; } }
        return `${e[1]} near ${best ? `${best.name} (${fmtDist(bd)} away)` : "an unnamed spot"}`;
      });
      if (named.length) add(`Where ${name} is recorded most, by 170 m cell: ${named.join("; ")}.`, `#map/${s.scientific_name}`, `${name} on the map`);
    }
  }

  for (const l of places) {
    const near = nearbySpecies(st.cells, l.lon, l.lat);
    add(`${l.name} (${l.kind}${l.ele_m ? `, ${Math.round(l.ele_m)} m` : ""})${l.summary?.extract ? `: ${l.summary.extract.slice(0, 260)}` : ""}`, l.summary?.url ?? l.url ?? undefined, l.name);
    if (near.list.length) add(`Recorded within 2.5 km of ${l.name}: ${near.list.map((n) => `${n.common ?? n.species} ${n.count}`).join(", ")} (${near.total.toLocaleString()} sightings in ${near.cells} cells).`, undefined, `Around ${l.name}`);
    const th = thingsNear(st.amenities, l.lon, l.lat);
    if (intents.includes("hike") && (th.trails.length || th.hike.length)) add(`Trails near ${l.name}: ${[...th.trails, ...th.hike].slice(0, 5).map((t) => `${t.label} (${t.detail})`).join("; ")}.`, undefined, `Trails near ${l.name}`);
    if (intents.includes("camp") && (th.camp.length || th.stay.length)) add(`Camping and lodging near ${l.name}: ${[...th.camp, ...th.stay].slice(0, 5).map((t) => `${t.label} (${t.detail})`).join("; ")}.`, undefined, `Camping near ${l.name}`);
  }

  if (month >= 0) {
    const ranked = all.filter((s) => s.months.reduce((a, b) => a + b, 0) >= 30 && !s.suppression)
      .map((s) => { const t = s.months.reduce((a, b) => a + b, 0); return { s, r: (s.months[month] / t) / (effort[month] / eTotal), n: s.months[month] }; })
      .filter((x) => x.n >= 10).sort((a, b) => b.r - a.r).slice(0, 6);
    if (ranked.length) add(`Species seen more than usual in ${MONTHS[month]} (share of their sightings vs everyone's): ${ranked.map((x) => `${x.s.common_name ?? x.s.scientific_name} ${x.r.toFixed(1)}× (${x.n} in ${MONTHS[month]})`).join(", ")}.`, undefined, `${MONTHS[month]}`);
  }
  if (intents.includes("camp") && places.length === 0 && st.amenities) {
    const camps = st.amenities.items.filter((i) => i.kind === "camp" && i.named && i.tags.backcountry !== "yes").slice(0, 8);
    if (camps.length) add(`Named campgrounds in the data: ${camps.map((c) => `${c.name}${c.tags.capacity ? ` (${c.tags.capacity} sites)` : ""}${c.tags.reservation ? `, reservation ${c.tags.reservation}` : ""}`).join("; ")}. Fees, seasons and reservations as tagged in OpenStreetMap; check nps.gov.`, undefined, "Campgrounds");
  }
  if (intents.includes("hike") && places.length === 0 && st.amenities) {
    const trails = st.amenities.trails.slice(0, 8);
    if (trails.length) add(`Longest named trails: ${trails.map((t) => `${t.name} ${(t.length_m / 1000).toFixed(1)} km`).join(", ")}.`, undefined, "Trails");
  }
...
export async function planFacts(g: Grounding): Promise<Fact[]> {
  ...
  for (const s of g.species) {
    ...
    const top = st.cells.features.filter((f) => !f.properties.coarsened).map((f) => ({ f, e: f.properties.sp.find((x) => x[0] === idx) }))
      .filter((x): x is { f: typeof x.f; e: NonNullable<typeof x.e> } => !!x.e).sort((a, b) => b.e[1] - a.e[1]).slice(0, 2);
    for (const { f, e } of top) {
      const ring = f.geometry.coordinates[0]; let cx = 0, cy = 0; for (let i = 0; i < ring.length - 1; i++) { cx += ring[i][0]; cy += ring[i][1]; } cx /= ring.length - 1; cy /= ring.length - 1;
      sites.push({ id: `cell:${f.properties.cell}`, label: `${s.common_name ?? s.scientific_name} spot (${e[1]} sightings)`, lon: cx, lat: cy, kind: "cell" });
    }
  }
  for (const p of g.places.slice(1)) sites.push({ id: p.id, label: p.name, lon: p.lon, lat: p.lat, kind: "landmark" });
  if (sites.length === 0) for (const s of stops.slice(1, 5)) sites.push({ id: s.id, label: s.name, lon: s.lon, lat: s.lat, kind: "stop" });
  ...
  const legs = plan.legs.map((l, i) => `${i + 1}. ${l.to.label}: ${fmtKm(l.distanceM)}, about ${fmtTime(l.seconds)} from ${l.from.label}`);
  return [{ n: 0, text: `A driving plan from ${start.name} over the park's roads (shortest order, ${fmtKm(plan.distanceM)} and about ${fmtTime(plan.seconds)} in total, assuming 35 mph and five minutes a stop): ${legs.join("; ")}.${plan.unreachable.length ? ` Not on the road network: ${plan.unreachable.map((u) => u.label).join(", ")}.` : ""}`, label: "Plan" }];
}

export interface Answer { text: string; facts: Fact[]; cited: number[]; uncited_numbers: string[]; ms: number; grounding: Grounding; }

const SYSTEM = (park: string) => `You are the guide for parkwild, a map of wildlife sightings in ${park}. Answer using ONLY the numbered FACTS. Every sentence must rest on a fact, and you cite it with its number in square brackets, like [2]. Never add species, places, numbers, dates or history that are not in the facts. If the facts do not answer the question, say "The data doesn't say" and name what the facts do cover. Write plainly in 3 to 6 short sentences, no headings, no bullet points, no preamble.`;
...
export async function ask(question: string, onToken?: (partial: string) => void): Promise<Answer> {
  ...
  if (g.intents.includes("plan")) {
    for (const f of await planFacts(g)) { f.n = g.facts.length + 1; g.facts.push(f); }
  }
  ...
  text = text.trim();
  const cited = [...new Set([...text.matchAll(/\[(\d+)\]/g)].map((m) => +m[1]))].filter((n) => g.facts.some((f) => f.n === n));
  ...
  const uncited_numbers = [...new Set((text.replace(/\[\d+\]/g, "").match(/\d[\d,\.]*/g) ?? []).map(clean))]
    .filter((n) => !factNums.has(n) && n.replace(/[^\d]/g, "").length >= 3);
...
export const EVAL_QUESTIONS = [
  "Where are bison seen most?", "When is the best month to see elk?", "How many species have been recorded here?",
  "Where can I camp near Lamar Valley?", "What trails are near Old Faithful?", "Plan a half day from Old Faithful to see bison and elk",
  "What animals are seen around Hayden Valley?", "Which birds are seen more than usual in October?", "How good is the camera model?",
  "Are there wolves on the map?", "What is the tallest peak?", "Tell me about Mammoth Hot Springs",
];

export async function evaluate(questions = EVAL_QUESTIONS): Promise<{ question: string; cited: number; facts: number; uncited_numbers: string[]; says_no_data: boolean; ms: number; text: string }[]> {
  const out = [];
  for (const q of questions) {
    const a = await ask(q);
    out.push({ question: q, cited: a.cited.length, facts: a.facts.length, uncited_numbers: a.uncited_numbers, says_no_data: /doesn't say|does not say/i.test(a.text), ms: a.ms, text: a.text });
  }
  return out;
}

if (typeof window !== "undefined") (window as unknown as { __parkwildAsk?: unknown }).__parkwildAsk = { ask, ground, evaluate, MODEL_ID };
