...
export function nearbySpecies(
  cells: CellsFile | null,
  lon: number,
  lat: number,
  radiusM = TOUR_RADIUS_M,
  limit = 6,
): Nearby {
  ...
  for (const f of cells.features) {
    ...
    for (const e of f.properties.sp) {
      const cur = agg.get(e[0]) ?? {
        species: cells.species_index[e[0]].n,
        common: cells.species_index[e[0]].c,
        count: 0,
        hv: 0,
        mp: 0,
      };
...
export function placeOf(it: NearItem): Place {
  return {
    id: it.id,
    kind: it.kind,
    name: it.label,
...
export function placeOfLandmark(l: Landmark): Place {
  ...
  return {
    ...
    lat: l.lat,
    wiki: title || null,
    tags: l.ele_m ? { ele: String(l.ele_m) } : undefined,
  };
}
...
export function thingsNear(a: AmenitiesFile | null, lon: number, lat: number): Things {
  ...
  const near = <T extends AmenityItem | TrailItem>(list: T[], radius: number) =>
    list
      .map((it) => ({ it, d: haversineM(lon, lat, it.lon, it.lat) }))
      .filter((x) => x.d <= radius)
      .sort((x, y) => x.d - y.d);
  ...
  const item = (it: AmenityItem, d: number, detail?: string): NearItem => ({
    ...
    label: it.name,
    detail:
      detail ?? `${it.sub}${it.tags.ele ? ` · ${Math.round(+it.tags.ele).toLocaleString()} m` : ""} · ${fmtDist(d)}`,
    lon: it.lon,
    lat: it.lat,
    distM: d,
    wiki: wikiOf(it),
  ...
  const items = a.items;
  const features = near(
    items.filter((i) => i.kind === "feature"),
    NEAR_M,
  )
    .slice(0, PER_GROUP)
    .map(({ it, d }) => item(it, d));
  const hike = near(
    items.filter((i) => i.kind === "trailhead"),
    NEAR_M,
  )
    .slice(0, PER_GROUP)
    .map(({ it, d }) => item(it, d, `trailhead · ${fmtDist(d)}`));
  const trails = near(a.trails, NEAR_M)
    .sort((x, y) => y.it.length_m - x.it.length_m)
    .slice(0, PER_GROUP)
    .map(({ it, d }) => ({
      id: it.id,
      kind: "trail",
      label: it.name,
      detail: `${(it.length_m / 1000).toFixed(1)} km of trail · ${fmtDist(d)} away`,
      lon: it.lon,
      lat: it.lat,
      distM: d,
      lengthM: it.length_m,
  ...
  const camp = near(
    items.filter((i) => i.kind === "camp"),
    CAMP_M,
  )
    .slice(0, PER_GROUP)
    .map(({ it, d }) => item(it, d, campDetail(it, d)));
  const stay = near(
    items.filter((i) => i.kind === "stay"),
    CAMP_M,
  )
    .slice(0, 3)
    .map(({ it, d }) => item(it, d, `${it.sub} · ${fmtDist(d)}`));
  const facilities = near(
    items.filter((i) => ["viewpoint", "picnic", "info", "boat"].includes(i.kind)),
    NEAR_M,
  )
    .slice(0, PER_GROUP)
...
function distToLineM(lon: number, lat: number, line: number[][]): number {
  const ky = 110_540,
    kx = 111_320 * Math.cos((lat * Math.PI) / 180);
  let best = Infinity;
  for (let i = 1; i < line.length; i++) {
    const ax = (line[i - 1][0] - lon) * kx,
      ay = (line[i - 1][1] - lat) * ky,
      bx = (line[i][0] - lon) * kx,
      by = (line[i][1] - lat) * ky;
    const dx = bx - ax,
      dy = by - ay,
...
export function speciesNearLines(
  cells: CellsFile | null,
  lines: number[][][],
  bufferM = TRAIL_BUFFER_M,
  limit = 8,
): Nearby {
  ...
  const dLat = bufferM / 110_540,
    dLon = bufferM / (111_320 * Math.cos((((s + n) / 2) * Math.PI) / 180));
  const agg = new Map<number, NearbySpecies>();
  const cellIds: string[] = [];
  let total = 0,
    count = 0;
  for (const f of cells.features) {
    ...
    for (const en of f.properties.sp) {
      const cur = agg.get(en[0]) ?? {
        species: cells.species_index[en[0]].n,
        common: cells.species_index[en[0]].c,
        count: 0,
        hv: 0,
        mp: 0,
      };
...
export function pointAt(rs: Resampled, d: number): [number, number] {
  ...
  while (hi - lo > 1) {
    const mid = (lo + hi) >> 1;
    if (rs.cum[mid] <= d) lo = mid;
    else hi = mid;
  }
  const span = rs.cum[hi] - rs.cum[lo] || 1,
    f = (d - rs.cum[lo]) / span;
  const a = rs.pts[lo],
    b = rs.pts[hi];
