...
function wigglyRoad(n = 400): number[][] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    pts.push([x + 0.00008 + rnd() * 0.0009, y + (rnd() - 0.5) * 0.0004]);
  }
  return pts;
}
...
test("the cruise climbs with the leg and stays inside its bounds", () => {
  const short = legDurationMs(300),
    long = legDurationMs(46_000);
  assert.equal(short, DRIVE_MIN_MS);
  assert.equal(long, DRIVE_MAX_MS);
  const zShort = cruiseZoom(300, short, 37),
    zLong = cruiseZoom(46_000, long, 37);
