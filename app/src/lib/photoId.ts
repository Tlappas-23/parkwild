...
type Classifier = (
  image: string,
  labels: string[],
  opts: { hypothesis_template: string; top_k?: number },
      ...
      .then(async (tf) => {
        ...
        const p = await tf.pipeline("zero-shot-image-classification", CLIP_MODEL, {
          ...
          progress_callback: (ev: { status?: string; progress?: number; file?: string }) =>
            onProgress(`${ev.status ?? ""} ${ev.file ?? ""}`.trim(), (ev.progress ?? 0) / 100),
        });
...
export async function suggestSpecies(file: Blob): Promise<Suggestion[]> {
  ...
  const list = (st.species?.species ?? [])
    .filter((s) => s.common_name && s.suppression?.action !== "exclude")
    .slice(0, MAX_LABELS);
  ...
  try {
    ...
    return out.map((o) => ({
      species: list.find((s) => s.common_name === o.label)?.scientific_name ?? o.label,
      common: o.label,
      score: o.score,
    }));
  } finally {
