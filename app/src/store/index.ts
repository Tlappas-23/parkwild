...
import { availableParks, loadCellPhotos, loadPark, loadPlaces, loadRoads, loadSpeciesIndex } from "../data/loaders";
import { MAX_SITES, planRoute, routerFor, type Mode, type PlanResult, type Site } from "../lib/routing";
...
function initialPage(): Page {
  try {
    return new URLSearchParams(window.location.search).has("park") ? "map" : "home";
  } catch {
    return "home";
  }
}
function nameOf(key: string): string {
  return PARKS.find((p) => p.key === key)?.name ?? key;
}
function readBasemap(): Basemap {
  try {
    return localStorage.getItem("parkwild:basemap") === "satellite" ? "satellite" : "terrain";
  } catch {
    return "terrain";
  }
}
...
export const useStore = create<State>((set, get) => ({
  ...
  selectPlace: (selectedPlace) => {
    set({ selectedPlace, selectedCell: selectedPlace ? null : get().selectedCell });
    if (selectedPlace) {
      void get().ensureCellPhotos();
      if (selectedPlace.kind === "trail") void get().ensureRoads();
    }
  },
  ...
  speciesScope: "park",
  setSpeciesScope: (speciesScope) => {
    set({ speciesScope });
    if (speciesScope === "all") void get().ensureSpeciesIndex();
  },
  speciesIndex: null,
  ...
  ensureSpeciesIndex: async () => {
    ...
    try {
      set({ speciesIndex: await loadSpeciesIndex(), speciesIndexError: null });
    } catch (e) {
      set({ speciesIndexError: (e as Error).message });
    }
  },
  focusCell: null,
  ...
  driveMode: (() => {
    try {
      return localStorage.getItem("parkwild:drive") !== "off";
    } catch {
      return true;
    }
  })(),
  setDriveMode: (driveMode) => {
    set({ driveMode });
    try {
      localStorage.setItem("parkwild:drive", driveMode ? "on" : "off");
    } catch {
      /* ignore */
    }
  },
  ...
  setPage: (page) =>
    set({
      page,
      selectedSpecies: page === "species" ? get().selectedSpecies : null,
      selectedPlaceId: page === "places" ? get().selectedPlaceId : null,
    }),
  ...
  showCameraPass: () => {
    set({ page: "about" });
    setTimeout(
      () => document.getElementById("camera-pass")?.scrollIntoView({ behavior: "smooth", block: "start" }),
      60,
    );
  },
  ...
  selectCell: (selectedCell) => {
    set({ selectedCell, selectedPlace: selectedCell ? null : get().selectedPlace });
    if (selectedCell) void get().ensureCellPhotos();
  },
  ...
  setPark: (park) => {
    ...
    set({
      ...
      tour: NO_TOUR,
      roads: null,
      plan: { ...NO_PLAN, start: get().plan.start?.kind === "me" ? get().plan.start : null },
    });
    try {
      const u = new URL(window.location.href);
      u.searchParams.set("park", park);
  ...
  enterPark: (park) => {
    if (park !== get().park) get().setPark(park);
    set({ page: "map", selectedSpecies: null });
  },
  setBasemap: (basemap) => {
    set({ basemap });
    try {
      localStorage.setItem("parkwild:basemap", basemap);
    } catch {
      /* ignore */
    }
  },
  ...
  startTour: () =>
    set({
      tour: { active: true, stop: 0, playing: true },
      page: "map",
      selectedCell: null,
      terrain3d: true,
      tourPrevBasemap: get().tour.active ? get().tourPrevBasemap : get().basemap,
      basemap: "satellite",
      controlsBeforeTour: get().tour.active ? get().controlsBeforeTour : get().controlsOpen,
      controlsOpen: false,
    }),
  endTour: () =>
    set({
      tour: NO_TOUR,
      basemap: get().tourPrevBasemap ?? get().basemap,
      tourPrevBasemap: null,
      controlsOpen: get().controlsBeforeTour,
    }),
  tourGo: (stop) =>
    set({
      tour: { ...get().tour, active: true, stop, playing: get().tour.active ? get().tour.playing : true },
      page: "map",
      selectedCell: null,
      terrain3d: true,
      tourPrevBasemap: get().tour.active ? get().tourPrevBasemap : get().basemap,
  ...
  tourNext: () => {
    const n = get().landmarks?.tour.length ?? 0,
      t = get().tour;
    set({ tour: t.stop < n - 1 ? { ...t, stop: t.stop + 1 } : { ...t, playing: false } });
  },
  tourPrev: () => {
    const t = get().tour;
    if (t.stop > 0) set({ tour: { ...t, stop: t.stop - 1 } });
  },
  ...
  locate: () =>
    new Promise<void>((resolve) => {
      if (!("geolocation" in navigator)) {
        set({ locationError: "This browser has no location service." });
        resolve();
        return;
      }
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          set({
            ...
            plan: {
              ...get().plan,
              start: { id: "me", label: "My location", lon: location.lon, lat: location.lat, kind: "me" },
              result: null,
            },
          });
          resolve();
        ...
        (err) => {
          set({
            locationError: err.code === 1 ? "Location permission was refused." : "Location is unavailable right now.",
          });
          resolve();
        },
  ...
  addSite: (site) => {
    const p = get().plan;
    if (p.sites.some((s) => s.id === site.id) || p.sites.length >= MAX_SITES) {
      set({ plan: { ...p, open: true } });
      return;
    }
    set({ plan: { ...p, open: true, sites: [...p.sites, site], result: null } });
  },
  removeSite: (id) =>
    set({ plan: { ...get().plan, sites: get().plan.sites.filter((s) => s.id !== id), result: null } }),
  ...
  computePlan: async () => {
    ...
    if (!roads) {
      set({ plan: { ...get().plan, busy: false, error: "No road data for this park yet." } });
      return;
    }
    try {
...
export function speciesMatches(
  s: { common_name: string | null; other_names?: string[]; scientific_name: string; aliases: string[] },
  q: string,
): boolean {
  const needle = q.trim().toLowerCase();
  if (!needle) return true;
  return [s.common_name ?? "", ...(s.other_names ?? []), s.scientific_name, ...s.aliases].some((n) =>
...
if (typeof window !== "undefined")
  (window as unknown as { __parkwildStore?: typeof useStore }).__parkwildStore = useStore;
