...
function make(hosts: string[], id: number, ext: string, host: number, observer: string, license: string, obs: number, date: string | null, species: string | null, cell: string | null): Photo {
  return {
    id, observer, license, observationId: obs, date, species, cell,
...
export function speciesPhotos(file: PhotosSpeciesFile | null, species: string): Photo[] {
  if (!file) return [];
  return (file.species[species] ?? []).map((p: SpeciesPhoto) => make(file.photo_hosts, p.i, p.e, p.h, p.o, p.l, p.obs, p.d, species, p.c));
}

export function cellPhotos(file: PhotosCellsFile | null, cell: string): Photo[] {
  if (!file) return [];
  return (file.cells[cell] ?? []).map((e: CellPhotoEntry) => make(file.photo_hosts, e[1], e[2], e[3], e[4], e[5], e[6], e[7], file.species_index[e[0]] ?? null, cell));
}
