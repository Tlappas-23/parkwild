...
import { directionsUrl, FAR_SNAP_M, fmtKm, fmtTime, MAX_SITES, SPEED_DRIVE_MS, type Site } from "../lib/routing";
...
export default function PlanPanel() {
  ...
  const found = useMemo(() => {
    ...
    const lm: Found[] = (landmarks?.landmarks ?? [])
      .filter((l) => l.name.toLowerCase().includes(n))
      .map((l) => ({ id: l.id, name: l.name, kind: l.kind, lon: l.lon, lat: l.lat, tour: l.tour }));
    const am: Found[] = (amenities?.items ?? [])
      .filter((i) => i.named && i.name.toLowerCase().includes(n))
      .map((i) => ({
        id: i.id,
        name: i.name,
        kind: i.kind === "info" ? "visitor centre" : i.sub,
        lon: i.lon,
        lat: i.lat,
      }));
    ...
    return cells.features
      ...
      .map(({ f, e }) => {
        ...
        return {
          ...
          label: `${common} hotspot · ${e[1].toLocaleString()} sightings`,
          lon,
          lat,
          kind: "cell" as const,
        };
  ...
  return (
    <div className="plan" role="region" aria-label="Plan a visit">
      <div className="plan-head">
        <strong>Plan a visit</strong>
        <button className="icon-btn" onClick={closePlan} aria-label="Close planner">
      ...
      <div className="plan-row">
        ...
        <button
          className={"toggle small-btn" + (startIsMe ? " on" : "")}
          onClick={() => void locate()}
          aria-pressed={startIsMe}
        >
          <LocateFixed className="ico" aria-hidden="true" /> My location
        </button>
        <select
          aria-label="Or start at a place"
          value={plan.start && !startIsMe ? plan.start.id : ""}
          onChange={(e) => {
            const l = landmarks?.landmarks.find((x) => x.id === e.target.value);
            setPlanStart(l ? siteOf(l) : null);
          }}
        >
      ...
      {startIsMe && location && (
        <p className="muted small plan-note">
          Position known to about {Math.round(location.accuracyM).toLocaleString()} m.
        </p>
      )}
      ...
      <div className="plan-row">
        ...
        <div className="seg" role="group" aria-label="Travel mode">
          <button
            className={plan.mode === "drive" ? "on" : ""}
            aria-pressed={plan.mode === "drive"}
            onClick={() => setPlanMode("drive")}
          >
            Drive
          </button>
          <button
            className={plan.mode === "hike" ? "on" : ""}
            aria-pressed={plan.mode === "hike"}
            onClick={() => setPlanMode("hike")}
          >
            Hike
          </button>
        </div>
      ...
      <div className="plan-row wrap">
        ...
        {stops.length > 0 && (
          <button className="ghost small-btn" onClick={() => stops.forEach((s) => addSite(siteOf(s)))}>
            <Plus className="ico" aria-hidden="true" /> all tour stops
          </button>
        )}
        {hotspots.map((h) => (
          <button key={h.id} className="ghost small-btn" onClick={() => addSite(h)}>
            + {h.label}
          </button>
        ))}
      </div>
      ...
      {plan.sites.length > 0 && (
        <ol className="sites" aria-label="Places to see">
          {plan.sites.map((s) => (
            <li key={s.id}>
              <span>{s.label}</span>
              <button className="chip-x" aria-label={`Remove ${s.label}`} onClick={() => removeSite(s.id)}>
                ×
              </button>
            </li>
          ))}
      ...
      <div className="plan-row">
        <button
          className="primary"
          disabled={!plan.start || plan.sites.length === 0 || plan.busy}
          onClick={() => void computePlan()}
        >
          {plan.busy ? "Planning…" : "Plan the best route"}
        </button>
        {r && (
      ...
      {r && (
        <div className="legs">
          <p className="legs-total">
            <strong>{fmtKm(r.distanceM)}</strong> · about {fmtTime(r.seconds)}{" "}
            {r.mode === "drive" ? "driving" : "on foot"} · {r.order.length} {r.order.length === 1 ? "stop" : "stops"}
          </p>
          <ol>
            {r.legs.map((l, i) => (
              <li key={l.to.id}>
                ...
                <span className="leg-body">
                  <strong>{l.to.label}</strong>
                  <br />
                  <span className="muted small">
                    {fmtKm(l.distanceM)} · {fmtTime(l.seconds)} from {l.from.kind === "me" ? "you" : l.from.label}
                    {(r.snapM[l.to.id] ?? 0) > FAR_SNAP_M
                      ? `; the nearest ${r.mode === "drive" ? "road" : "path"} is ${fmtKm(r.snapM[l.to.id])} from the point`
                      : ""}
                  </span>
                  {" · "}
                  <a className="small" href={directionsUrl(l.from, l.to)} target="_blank" rel="noreferrer">
                    turn-by-turn ↗
                  </a>
                </span>
              </li>
          ...
          {r.unreachable.length > 0 && (
            <p className="muted small">
              Not on the {r.mode === "drive" ? "road" : "road or trail"} network from there:{" "}
              {r.unreachable.map((u) => u.label).join(", ")}.
            </p>
          )}
          <p className="muted small">
            Routes follow OpenStreetMap roads{r.mode === "hike" ? " and trails" : ""} and know nothing of closures or
            seasons; check{" "}
            <a href="https://www.nps.gov/planyourvisit/index.htm" target="_blank" rel="noreferrer">
              nps.gov
            </a>{" "}
            before you go. Times assume {r.mode === "drive" ? `${Math.round(SPEED_DRIVE_MS * 2.237)} mph` : "5 km/h"}{" "}
            plus five minutes a stop.
          </p>
        </div>
      )}
    </div>
