...
import { fmtDist, nearbySpecies, photoNear, speciesNearLines, TRAIL_BUFFER_M, trailLines } from "../lib/tour";
...
export default function PlaceDetail() {
  ...
  const nearby = useMemo(() => {
    ...
    return p.kind === "trail"
      ? lines.length
        ? speciesNearLines(cells, lines)
        : null
      : nearbySpecies(cells, p.lon, p.lat, POINT_RADIUS_M, 8);
  }, [cells, p, lines]);
  ...
  useEffect(() => {
    ...
    setPhotos(undefined);
    let live = true;
    (p.wiki ? wikiSummary(p.wiki) : wikiFind(p.name, parkName))
      .then((s) => {
  ...
  if (p.tags?.reservation)
    facts.push(p.tags.reservation === "no" ? "first come, first served" : `reservation ${p.tags.reservation}`);
  ...
  const kindLabel =
    p.kind === "info" ? "visitor centre" : p.kind === "stay" ? "lodging" : p.kind === "camp" ? "camping" : p.kind;
  ...
  return (
    <aside className="drawer place" aria-label="Place detail">
      ...
      <div className="place-actions">
        ...
        >
          <Plus className="ico" aria-hidden="true" /> Add to route
        </button>
        {p.tags?.website && (
          <a className="ghost small-btn as-btn" href={p.tags.website} target="_blank" rel="noreferrer">
            Website <ExternalLink className="ico" aria-hidden="true" />
          </a>
        )}
        {summary && (
          <a className="ghost small-btn as-btn" href={summary.url} target="_blank" rel="noreferrer">
            Wikipedia <ExternalLink className="ico" aria-hidden="true" />
          </a>
        )}
      </div>
      ...
      <section className="place-sec">
        ...
        {summary === undefined ? (
          ...
          <p className="place-text">
            {summary.extract} <span className="muted small nowrap">Wikipedia · {summary.title} · CC BY-SA 4.0</span>
          </p>
        ) : (
          <p className="muted small">
            No Wikipedia article for this {kindLabel}.
            {p.tags?.description ? ` OpenStreetMap notes: ${p.tags.description}` : ""}
          </p>
        )}
      </section>
      ...
      <section className="place-sec">
        <div className="eyebrow">
          {p.kind === "trail"
            ? `Recorded along the trail (within ${TRAIL_BUFFER_M} m)`
            : `Recorded within ${fmtDist(POINT_RADIUS_M)}`}
        </div>
        {p.kind === "trail" && !roads ? (
          <p className="muted small">Loading the trail…</p>
          ...
          <>
                ...
                return (
                  <li key={n.species}>
                    <button
                      ...
                      title={`${n.common ?? n.species}: ${n.count.toLocaleString()} sightings here`}
                    >
                      {hit ? (
                        <span className="thumb">
                          <img
                            src={hit.photo.url("square")}
                            alt={`${n.common ?? n.species} photographed by ${hit.photo.observer}`}
                            loading="lazy"
                          />
            ...
            <p className="muted small">
              {nearby.total.toLocaleString()} sightings in {nearby.cells.toLocaleString()} cells. Sensitive species are
              never listed by place.{" "}
              {nearby.list[0] && (
                <button
                  className="link small"
                  onClick={() => {
                    setSpeciesFilter(nearby.list[0].species);
                    setPage("map");
                    selectPlace(null);
                  }}
                >
                  Show {nearby.list[0].common ?? nearby.list[0].species} on the map
                </button>
              )}
            </p>
          </>
      ...
      <section className="place-sec">
        ...
        {photos === undefined ? (
          <p className="muted small">Looking on Wikimedia Commons…</p>
        ) : photos.length > 0 ? (
          <div className="photo-grid">
            {photos.map((ph) => (
              <figure key={ph.url}>
                <a href={ph.page} target="_blank" rel="noreferrer">
                  <img src={ph.url} alt={`${p.name}, photographed by ${ph.artist}`} loading="lazy" />
                </a>
                <figcaption className="credit compact">
                  {ph.artist} · {ph.license} · {ph.distM} m
                </figcaption>
              </figure>
            ))}
          </div>
      ...
      <p className="muted small pad">
        Place data © OpenStreetMap contributors. Text from Wikipedia, photographs from Wikimedia Commons, each under the
        licence shown.
      </p>
    </aside>
  );
