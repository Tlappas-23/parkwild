...
export default function Tour() {
  ...
  const group = (title: string, list: NearItem[]) =>
    list.length > 0 && (
        ...
        <ul className="todo-list">
          {list.map((it) => (
            <li key={it.id}>
              ...
              <button className="todo-body as-link" onClick={() => selectPlace(placeOf(it))} title="Open this place">
                <strong>{it.label}</strong>
                <br />
                <span className="muted small">{it.detail}</span>
              </button>
              <button
                className="chip-x add"
                aria-label={`Add ${it.label} to route`}
                title="Add to route"
                onClick={() => addSite({ id: it.id, label: it.label, lon: it.lon, lat: it.lat, kind: "landmark" })}
              >
  ...
  useEffect(() => {
    if (tour.active) void ensureCellPhotos();
  }, [tour.active, ensureCellPhotos]);
  ...
  useEffect(() => {
    ...
    setProgress(0);
  }, [tour.active, tour.stop, tourDrive]);
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      if (tour.stop < stops.length - 1) tourNext();
      else tourPlay(false);
    }, remaining);
    const iv = setInterval(
      () => setProgress(Math.min(1, (performance.now() - arrivedAt.current) / TOUR_DWELL_MS)),
      250,
    );
    return () => {
      clearTimeout(t);
  ...
  if (minimised) {
    return (
      <section className="tour min" aria-label="Virtual tour" aria-live="polite">
        {progressBar}
        <button className="icon-btn" onClick={tourPrev} disabled={tour.stop === 0} aria-label="Previous stop">
        ...
        <button className="tour-min-title" onClick={() => setMinimised(false)} title="Show the stop">
          <span className="eyebrow">
            {tour.stop + 1}/{stops.length}
          </span>{" "}
          {stop.name}
        </button>
        <button
          className="icon-btn"
          onClick={last ? () => tourGo(0) : tourNext}
          aria-label={last ? "Back to the first stop" : "Next stop"}
        >
          {last ? (
        ...
        <button className="icon-btn" onClick={endTour} aria-label="Exit tour">
  ...
  return (
    <section className={"tour" + (expanded ? " expanded" : "")} aria-label="Virtual tour" aria-live="polite">
      ...
      <div className="tour-main">
        ...
        {stop.summary?.extract ? (
          <p className="tour-text">
            {stop.summary.extract}{" "}
            <a href={stop.summary.url} target="_blank" rel="noreferrer" className="muted small nowrap">
              Wikipedia, {stop.summary.licence}
            </a>
          </p>
        ) : stop.url ? (
          <p className="tour-text muted">
            <a href={stop.url} target="_blank" rel="noreferrer">
              About {stop.name} on Wikipedia
            </a>
          </p>
        ) : null}
      </div>
      ...
      <div className="tour-tabs" role="tablist" aria-label="Stop details">
        <button
          role="tab"
          aria-selected={tourTab === "wildlife"}
          className={tourTab === "wildlife" ? "on" : ""}
          onClick={() => setTourTab("wildlife")}
        >
          Wildlife
        </button>
        <button
          role="tab"
          aria-selected={tourTab === "todo"}
          className={tourTab === "todo" ? "on" : ""}
          onClick={() => setTourTab("todo")}
        >
          Things to do{things && things.total ? ` · ${things.total}` : ""}
        </button>
        {((stop.photos?.length ?? 0) > 0 || stop.street) && (
          <button
            role="tab"
            aria-selected={tourTab === "photos"}
            className={tourTab === "photos" ? "on" : ""}
            onClick={() => setTourTab("photos")}
          >
            Photos{stop.photos?.length ? ` · ${stop.photos.length}` : ""}
          </button>
        )}
      </div>
        ...
        <div className="tour-photos">
          {stop.street && (
            <a className="street-link" href={stop.street.url} target="_blank" rel="noreferrer">
              <span>
                <Compass className="ico" aria-hidden="true" /> Look around from here on Mapillary
              </span>
              <span className="muted small">
                {stop.street.is_pano ? "360° photo" : "street-level photo"} · {stop.street.dist_m} m away
                {stop.street.captured_at ? ` · ${stop.street.captured_at.slice(0, 4)}` : ""}
                {stop.street.username ? ` · © ${stop.street.username}` : ""} · {stop.street.license}
              </span>
            </a>
          )}
              ...
              {stop.photos!.map((p) => (
                <figure key={p.url}>
                  <a href={p.page} target="_blank" rel="noreferrer">
                    <img src={p.url} alt={`${stop.name}, photographed by ${p.artist}`} loading="lazy" />
                  </a>
                  <figcaption className="credit compact">
                    {p.artist} · {p.license} · {p.dist_m} m
                  </figcaption>
                </figure>
              ))}
            </div>
          ...
          <p className="muted small tour-foot">
            Photographs from Wikimedia Commons under the licence printed on each; street-level imagery on Mapillary, CC
            BY-SA 4.0.
          </p>
        </div>
      )}
        ...
        <div className="tour-todo">
          {things && things.total > 0 ? (
            <>
              ...
              <p className="muted small tour-foot">
                OpenStreetMap data; fees, capacities and seasons as tagged there. Check nps.gov for current status and
                reservations.
              </p>
            </>
          ) : (
            <p className="muted small">
              {amenities
                ? "Nothing tagged within reach of this stop."
                : "Things to do are not exported for this park yet."}
            </p>
          )}
        </div>
      )}
      ...
      <div className="tour-wild" hidden={tourTab !== "wildlife"}>
        {nearby && nearby.list.length > 0 ? (
          <ul className="tour-species" aria-label={`Recorded within ${TOUR_RADIUS_M / 1000} km`}>
            {nearby.list.slice(0, expanded ? 6 : 3).map((n) => {
              // one row unless Details is open
              ...
              return (
                <li key={n.species}>
                  <button
                    className="tour-card"
                    onClick={() => selectSpecies(n.species)}
                  ...
                  >
                    {photo ? (
                      <span className="thumb">
                        <img
                          src={photo.url("square")}
                          alt={`${n.common ?? n.species} photographed by ${photo.observer}`}
                          loading="lazy"
                        />
        ...
        {nearby && (
          <p className="muted small tour-foot">
            {nearby.total.toLocaleString()} sightings in {nearby.cells.toLocaleString()} cells within{" "}
            {TOUR_RADIUS_M / 1000} km. Sensitive species are never listed by landmark.
          </p>
        )}
      </div>
      ...
      <div className="tour-nav">
        ...
        <button className="primary" onClick={() => tourPlay(!tour.playing)} aria-pressed={tour.playing}>
          {tour.playing ? <Pause className="ico" aria-hidden="true" /> : <Play className="ico" aria-hidden="true" />}
          {tour.playing ? "Pause" : last ? "Replay" : "Play"}
        </button>
        <button
          className="icon-btn"
          onClick={last ? () => tourGo(0) : tourNext}
        ...
        <button
          ...
          aria-label={expanded ? "Less detail" : "More detail"}
          title={expanded ? "Less" : "Details"}
        >
          {expanded ? (
            <Minimize2 className="ico" aria-hidden="true" />
          ) : (
            <Maximize2 className="ico" aria-hidden="true" />
          )}
        </button>
        <button
          className={"icon-btn" + (driveMode ? " on" : "")}
          onClick={() => setDriveMode(!driveMode)}
          aria-pressed={driveMode}
          aria-label={
            driveMode
              ? "Between stops: drive the road (tap to fly instead)"
              : "Between stops: fly (tap to drive the road)"
          }
          title={driveMode ? "Driving the road between stops" : "Flying between stops"}
        >
          {driveMode ? <Car className="ico" aria-hidden="true" /> : <Plane className="ico" aria-hidden="true" />}
        </button>
        <button className="icon-btn" onClick={() => setMinimised(true)} aria-label="Shrink to a strip" title="Minimise">
          <Minus className="ico" aria-hidden="true" />
        </button>
        <button
          className="icon-btn"
          onClick={() => addSite({ id: stop.id, label: stop.name, lon: stop.lon, lat: stop.lat, kind: "stop" })}
          aria-label="Add this stop to a route"
          title="Add to route"
        >
          <Plus className="ico" aria-hidden="true" />
        </button>
        <button className="icon-btn" onClick={endTour} aria-label="Exit tour" title="Exit tour">
          <X className="ico" aria-hidden="true" />
        </button>
        <div className="dots" role="tablist" aria-label="Stops">
          {stops.map((s, i) => (
            <button
              key={s.id}
              role="tab"
              aria-selected={i === tour.stop}
              className={i === tour.stop ? "on" : ""}
