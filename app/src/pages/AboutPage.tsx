...
export default function AboutPage() {
  ...
  return (
    <article className="page prose">
      ...
      <p className="lede">
        Where and when animals have been recorded in {parkName}, from public observations by people who were there, aggregated into hexagonal cells about 170 m across.
        It is a record of what people reported, not a survey of what lives here.
      </p>

      <h2>What you are looking at</h2>
      <p>
        Each hexagon holds every sighting whose coordinates fall inside it; the deeper the blue, the more sightings. Sensitive species are drawn in much larger cells or not at all.
        {modelCells
      ...
      <p>
        {species ? `${short} holds ${total.toLocaleString()} sightings of ${species.species.length} species${years ? `, recorded ${years}` : ""}.` : ""}{" "}
        <span className="badge human">verified</span> means iNaturalist research grade (at least two people agreed on the identification) or another GBIF dataset with its own review.
        The photographs on this site are the observers' own, shown under the licence each chose and credited beside every image.
      </p>
      ...
      <ul>
        ...
        <li><strong>Seasons reflect visitors.</strong> Most observations are from June to August because that is when people are in the park. The species pages show a second figure, the species' share of each month against everyone's, which separates the animal's season from the visitors'.</li>
      ...
      <p>
        Some species are deliberately not mapped, or mapped only at about 3 km cells, because a precise public map of them would help the wrong people.
        {excluded.length > 0 && <> Not shown: {named(excluded)}.</>}
        {coarsened.length > 0 && <> Shown coarsely: {named(coarsened)}.</>}{" "}
        Where a source already obscures a location, that record is counted but never mapped, its photographs never appear in a cell, and it is never listed by landmark or trail.
      </p>
      ...
      {passRan.length > 0 ? (
        <>
          <p>
            A computer-vision model (SpeciesNet: a detector and a species classifier) looked for animals in street-level photographs taken from park roads (Mapillary, CC BY-SA).
            It is a supplementary layer, kept apart from human sightings on purpose and drawn amber: it sees only what a car sees, mostly in June, and its measured precision is below half at the threshold used. Its counts are small and shown as they are.
          </p>
          ...
          <p className="muted small">
            ...
            Precision is the share of reviewed detections that a person confirmed as an animal. Detections are positioned at the camera, about {cameraPass!.thresholds.range_m} m from the animal.
          </p>
        </>
      ) : (
        <p>
          <strong>No computer-vision pass has run in {short}.</strong> Every count, cell and photograph here comes from people.
          {passQueued.length > 0 ? ` A pass is queued for ${passQueued.map((c) => c.name.split(",")[0]).join(" and ")}; until it runs and is reviewed, nothing on this map is machine-detected.` : ""}
        </p>
      )}
      <h3>Ask, on your device</h3>
      <p>
        The Ask page can run a small language model in your browser, but only after you press Enable and download it (about 1 GB, once). Until then no language model is involved anywhere on this site:
        the tour text is the opening of each place's Wikipedia article, the numbers are counts, the routes are shortest paths over OpenStreetMap roads.
      ...
      {bias ? (
        ...
        <p className="muted">Not measured for {short}: these figures compare street-level imagery with human sightings, and no imagery corridor has been pulled here.</p>
      )}
      ...
      <ul>
        ...
        <li>Street-level imagery: Mapillary, CC BY-SA 4.0; image ID and contributor stored with every derived record. Images themselves are not redistributed; "look around" links open Mapillary.</li>
        ...
        <li>Landmarks, trails, campsites and facilities: OpenStreetMap, ODbL{landmarks ? ` (${landmarks.landmarks.length} landmarks in ${short})` : ""}. Place descriptions are the opening of each Wikipedia article, CC BY-SA 4.0, linked beside the text. Photographs of places: Wikimedia Commons, each under the licence printed beside it. The park outline is the iNaturalist place polygon the sightings were filtered by.</li>
      </ul>

      <h2>How this site is built</h2>
      <p>
        Everything is static files: a Python pipeline bakes each park's data, a React app reads it, GitHub Pages serves it; there is no server and no account. Every data file is hashed and the app refuses one that does not match.
        The architecture, the user guide, the decision records and the experiment ledger are in the repository:{" "}
        <a href="https://github.com/Tlappas-23/parkwild/blob/main/docs/ARCHITECTURE.md" target="_blank" rel="noreferrer">architecture</a>,{" "}
        <a href="https://github.com/Tlappas-23/parkwild/blob/main/docs/USER-GUIDE.md" target="_blank" rel="noreferrer">how to use the site</a>,{" "}
        <a href="https://github.com/Tlappas-23/parkwild/blob/main/DECISIONS.md" target="_blank" rel="noreferrer">decisions</a>,{" "}
        <a href="https://github.com/Tlappas-23/parkwild/blob/main/EXPERIMENTS.md" target="_blank" rel="noreferrer">experiments</a>.
      </p>
      <p className="muted small">
