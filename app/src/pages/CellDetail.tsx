...
import type { CellFeature, Species } from "../types";
...
function observationsUrl(feature: CellFeature, taxon: Species | null): string {
  ...
  for (const [lng, lat] of feature.geometry.coordinates[0]) { w = Math.min(w, lng); e = Math.max(e, lng); s = Math.min(s, lat); n = Math.max(n, lat); }
  const q = new URLSearchParams({ nelat: n.toFixed(5), nelng: e.toFixed(5), swlat: s.toFixed(5), swlng: w.toFixed(5), quality_grade: "research", verifiable: "any" });
...
export default function CellDetail() {
  ...
  const allPhotos = useMemo(() => (selectedCell ? cellPhotos(photosCells, selectedCell) : []), [photosCells, selectedCell]);
  ...
  const focusPhotos = useMemo(() => {
    ...
    return [...allPhotos.filter((p) => p.species === speciesFilter), ...speciesPhotos(photosSpecies, speciesFilter).filter((p) => p.cell === selectedCell)]
    ...
    <aside className="drawer" aria-label="Cell detail">
      <div className="drawer-head">
        <div>
          ...
          {focus ? (
            <>
              <h2>{focus.count.toLocaleString()} {focus.common ?? focus.species} sighting{focus.count === 1 ? "" : "s"} here</h2>
              <div className="muted small">{focus.y0 ?? "?"} to {focus.y1 ?? "?"} · of {cell.count.toLocaleString()} sightings in this cell</div>
            </>
          ) : (
            <>
      ...
      {photos.length > 0 ? strip(photos, focus ? `Photographs of ${focus.common ?? focus.species} taken in this cell` : "Photographs taken in this cell") : (
        <p className="muted small pad">
          {!photosCells ? "Loading photographs…" : focus
      ...
      <a className="verify" href={observationsUrl(feature, focusMeta)} target="_blank" rel="noreferrer">
        See {focus ? `${focus.common ?? focus.species} observations` : "the observations"} in about this area on iNaturalist ↗
      </a>
