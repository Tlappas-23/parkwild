...
import { indexMatches } from "../lib/speciesSearch";
...
export default function SpeciesPage() {
  ...
  useEffect(() => {
    if (speciesScope === "all") void ensureSpeciesIndex();
  }, [speciesScope, ensureSpeciesIndex]);
  const list = useMemo(() => {
        ...
        <div>
          ...
          <p className="muted">
            {speciesScope === "park"
              ...
              : speciesIndex
                ? `${speciesIndex.species.length} species across ${parkCount} parks, ordered by sightings. Open one to see where it is seen most, park by park.`
                : "Loading every park…"}
          </p>
        </div>
        <div className="page-tools">
          <div className="seg" role="group" aria-label="Scope">
            <button className={speciesScope === "park" ? "active" : ""} onClick={() => setSpeciesScope("park")}>
              In {shortPark(parkName)}
            </button>
            <button className={speciesScope === "all" ? "active" : ""} onClick={() => setSpeciesScope("all")}>
              All parks
            </button>
          </div>
          <input
          ...
          <div className="seg" role="group" aria-label="Class">
            {(["all", "Mammalia", "Aves"] as const).map((c) => (
              <button key={c} className={cls === c ? "active" : ""} onClick={() => setCls(c)}>
                {c === "all" ? "All" : c === "Mammalia" ? "Mammals" : "Birds"}
              </button>
            ))}
          </div>
        </div>
      ...
      {speciesScope === "park" ? (
          ...
          {list.map((s, i) => {
            ...
            return (
              <button
                ...
                role="listitem"
                className="card"
                onClick={() => selectSpecies(s.scientific_name)}
              >
                <div className="card-media">
                  {photo ? (
                    ...
                    <div className="card-empty">{(s.common_name ?? s.scientific_name).slice(0, 1)}</div>
                  )}
                  {s.suppression && (
                    <span className="pill">
                      {s.suppression.action === "exclude" ? "not mapped" : "mapped coarsely"}
                    </span>
                  )}
                </div>
        ...
        <div className="rows" role="list">
            ...
            return (
              <button key={e.n} role="listitem" className="row" onClick={() => selectSpecies(e.n)}>
                ...
                <div className="row-parks" aria-label="Parks, most sightings first">
                  {shown.map(([k, p]) => (
                    <span key={k} className="chip-park">
                      {shortPark(speciesIndex?.parks[k]?.name ?? k)}
                      <b>{p.s.toLocaleString()}</b>
                    </span>
                  ))}
                  {parks.length > shown.length && (
                    <span className="muted small">+{parks.length - shown.length} more</span>
                  )}
                </div>
                <div className="row-total">
                  <strong>{e.total.toLocaleString()}</strong>
                  <span className="muted small">
                    in {parks.length} park{parks.length === 1 ? "" : "s"}
                  </span>
                </div>
              </button>
            );
          ...
          {rows.length > MAX_ROWS && (
            <p className="muted small">{rows.length - MAX_ROWS} more match; type to narrow the list.</p>
          )}
