...
export default function HomePage() {
  ...
  const card = (p: ParkCard) => (
    <button
      key={p.key}
      className={"park-card" + (p.status !== "live" ? " planned" : "") + (p.key === park ? " current" : "")}
      onClick={() => enterPark(p.key)}
      disabled={p.status !== "live"}
      aria-label={`${p.name}${p.status !== "live" ? ", coming soon" : ""}`}
    >
      ...
      <div className="park-body">
        ...
        {p.status === "live" ? (
          <p className="muted small">
            {p.species?.toLocaleString()} species · {p.sightings?.toLocaleString()} sightings · route planner
          </p>
        ) : (
        ...
        {p.hero && (
          <p className="credit compact">
            <a href={p.hero.page} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()}>
              Photo
            </a>
            : {p.hero.artist} · {p.hero.license}
          </p>
        )}
      </div>
    </button>
  ...
  return (
    <div className="page home">
      <div className="home-hero" style={heroPark?.hero ? { backgroundImage: `url(${heroPark.hero.url})` } : undefined}>
        ...
        {heroPark?.hero && (
          <p className="credit">
            {heroPark.name.replace(/ National Park$/, "")} ·{" "}
            <a href={heroPark.hero.page} target="_blank" rel="noreferrer">
              photo
            </a>{" "}
            {heroPark.hero.artist} · {heroPark.hero.license}
          </p>
        )}
      </div>
      {live.length === 0 && planned.length === 0 && seed.length === 0 && (
        <p className="muted">No park matches "{q}".</p>
      )}
      <div className="park-grid">{live.map(card)}</div>
      <h2 className="home-sub">Every park on the map</h2>
      <p className="muted small">
        Filled dots are open; hollow dots are being gathered; faint dots are not started. Click a filled one to go in.
      </p>
      <Suspense fallback={<div className="home-map placeholder">Loading the map…</div>}>
        <HomeMap parks={index.parks} />
      </Suspense>
      {planned.length > 0 && (
        <>
      ...
      {seed.length > 0 && (
        <>
          ...
          <p className="park-names">
            {seed.map((p) => `${p.name.replace(/ National Park.*$/, "")} (${p.state})`).join(" · ")}
          </p>
        </>
      )}
      <p className="muted small home-foot">
        Sightings: iNaturalist research-grade observations and other GBIF datasets. Photographs stay with their
        observers' licences and names. Park photographs: Wikimedia Commons, credited on each card. Nothing here is a
        survey; an empty place means nobody looked.
      </p>
    </div>
  );
}
