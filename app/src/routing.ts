...
import type { RoadsFile } from "./types";
...
export interface Site { id: string; label: string; lon: number; lat: number; kind: "stop" | "landmark" | "cell" | "me"; }
export interface Leg { from: Site; to: Site; distanceM: number; seconds: number; coords: number[][]; }
export interface PlanResult { mode: Mode; order: Site[]; legs: Leg[]; distanceM: number; seconds: number; unreachable: Site[]; snapM: Record<string, number>; }

const KIND_ROAD = 0;

function metres(lon1: number, lat1: number, lon2: number, lat2: number): number {
  const ky = 110_540, kx = 111_320 * Math.cos(((lat1 + lat2) / 2) * Math.PI / 180);
...
export interface Shortest { dist: Float64Array; prevNode: Int32Array; prevEdge: Int32Array; }
...
export class Router {
  ...
  constructor(readonly file: RoadsFile) {
    ...
    file.edges.forEach((e, i) => {
      ...
      if (kind === KIND_ROAD) { this.roadNode[a] = 1; this.roadNode[b] = 1; }
    });
  }
  ...
  shortest(source: number, mode: Mode): Shortest {
    ...
    const prevNode = new Int32Array(n).fill(-1), prevEdge = new Int32Array(n).fill(-1);
    const heap = new Heap();
    ...
    while (heap.size) {
      ...
      for (const [v, ei] of this.adj[u]) {
        ...
        const nd = d + e[2];
        if (nd < dist[v]) { dist[v] = nd; prevNode[v] = u; prevEdge[v] = ei; heap.push(nd, v); }
      }
    }
  ...
  path(s: Shortest, target: number): number[][] {
    ...
    while (s.prevEdge[v] >= 0) {
      const e = this.file.edges[s.prevEdge[v]], u = s.prevNode[v];
    ...
    parts.reverse();
    const out: number[][] = [];
    for (const p of parts) for (let i = 0; i < p.length; i++) { if (out.length && i === 0) continue; out.push(p[i]); }
    return out;
  }
}
...
export function routerFor(file: RoadsFile): Router {
  ...
  if (!r) { r = new Router(file); routers.set(file, r); }
  return r;
}
...
function orderExact(cost: number[][]): number[] {
  ...
  for (let mask = 1; mask <= full; mask++) {
    for (let j = 0; j < n; j++) {
      ...
      for (let k = 0; k < n; k++) {
        ...
        const nm = mask | (1 << k), c = dp[mask][j] + cost[j + 1][k + 1];
  ...
  const order: number[] = [];
  let mask = full, j = end;
  while (j >= 0) { order.push(j); const pj = par[mask][j]; mask &= ~(1 << j); j = pj; }
...
function orderHeuristic(cost: number[][]): number[] {
  ...
  while (left.size) {
    ...
    for (const j of left) if (cost[cur][j + 1] < bd) { bd = cost[cur][j + 1]; best = j; }
    order.push(best); left.delete(best); cur = best + 1;
  }
  ...
  while (improved) {
    ...
    for (let i = 0; i < n - 1; i++) for (let k = i + 1; k < n; k++) {
      ...
      if (total(cand) + 1e-9 < total(order)) { order.splice(0, n, ...cand); improved = true; }
    }
  }
  return order;
}
...
export function planRoute(router: Router, start: Site, sites: Site[], mode: Mode): PlanResult {
  ...
  const nodes = points.map((p) => { const s = router.snap(p.lon, p.lat, mode); snapM[p.id] = s.distM; return s.node; });
  const reachable: number[] = [];      // indices into points that found a node at all
  const unreachable: Site[] = [];
  points.forEach((p, i) => { if (i === 0 || nodes[i] >= 0) reachable.push(i); else unreachable.push(p); });
  ...
  for (let r = 1; r < reachable.length; r++) {
    if (shortest[0].dist[nodes[reachable[r]]] < Infinity) keep.push(reachable[r]); else unreachable.push(points[reachable[r]]);
  }
  ...
  for (const j of order) {
    ...
    legs.push({ from: points[keep[prev]], to: points[keep[to]], distanceM, seconds: distanceM / speed + STOP_OVERHEAD_S,
                coords: router.path(s, nodes[keep[to]]) });
    prev = to;
  }
  return { mode, order: order.map((j) => points[keep[j + 1]]), legs,
