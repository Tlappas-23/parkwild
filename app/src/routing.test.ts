...
const nodes = [[0, 0], [0.01, 0], [0.02, 0], [0.03, 0]];
const seg = (i: number, j: number, kind: 0 | 1): RoadsFile["edges"][number] => [i, j, 1113, kind, 0, 0, [nodes[i], nodes[j]]];
const roads: RoadsFile = { park: "test", fetched: "", attribution: "", names: ["Test Road"], nodes, edges: [seg(0, 1, 0), seg(1, 2, 0), seg(2, 3, 1)] };
...
test("driving uses roads only; hiking may take the trail", () => {
  ...
  const drive = r.shortest(0, "drive"), hike = r.shortest(0, "hike");
  assert.equal(drive.dist[2], 2226);
  assert.equal(drive.dist[3], Infinity);
  assert.equal(hike.dist[3], 3339);
  assert.deepEqual(r.path(hike, 3).map((c) => c[0]), [0, 0.01, 0.02, 0.03]);   // shared vertices stitched once
});
...
test("the plan visits the sites in the shortest order", () => {
  ...
  assert.deepEqual(plan.order.map((s) => s.id), ["near", "far"]);
