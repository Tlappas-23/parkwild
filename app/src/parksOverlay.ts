...
export function parksFC(parks: ParkCard[]): FeatureCollection {
  return {
    ...
    features: parks.filter((p) => p.center).map((p) => ({
      type: "Feature",
      properties: { key: p.key, name: p.name.replace(/ National Park.*$/, ""), state: p.state, status: p.status,
                    label: `${p.name.replace(/ National Park.*$/, "")}`, live: p.status === "live" },
...
export function liveBounds(parks: ParkCard[]): [[number, number], [number, number]] | null {
  ...
  for (const p of pool) { const [a, b, c, d] = p.bbox!; w = Math.min(w, a); e = Math.max(e, c); s = Math.min(s, b); n = Math.max(n, d); }
  return [[w, s], [e, n]];
}
...
export function addParksLayers(map: MLMap, beforeId?: string): void {
  ...
  map.addLayer({ id: "parks-dot", type: "circle", source: "parks",
    paint: { "circle-radius": ["case", ["get", "live"], 8, ["==", ["get", "status"], "planned"], 6, 4],
             "circle-color": ["case", ["get", "live"], "#2563eb", "#ffffff"],
             "circle-stroke-color": ["case", ["get", "live"], "#ffffff", "#475569"], "circle-stroke-width": ["case", ["get", "live"], 2, 1.5],
             "circle-opacity": ["case", ["==", ["get", "status"], "seed"], 0.55, 1] } }, beforeId);
  map.addLayer({ id: "parks-label", type: "symbol", source: "parks",
    layout: { "text-field": ["get", "label"], "text-font": ["Noto Sans Regular"], "text-size": ["case", ["get", "live"], 12.5, 11],
              "text-offset": [0, 0.9], "text-anchor": "top", "text-optional": true, "text-max-width": 8 },
    paint: { "text-color": ["case", ["get", "live"], "#12324a", "#4b5563"], "text-halo-color": "rgba(255,255,255,0.92)", "text-halo-width": 1.3,
             "text-opacity": ["case", ["==", ["get", "status"], "seed"], 0.8, 1] } }, beforeId);
}
