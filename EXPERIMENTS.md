...
- **What:** my review of the deployed app.
...
- **What:** I opened the live site after two deploys in quick succession and saw "species.json failed its integrity check".
...
- **What:** I filtered the map to Wapiti, tapped a cell on the Madison River and got all 29 species of the cell, three bison photographs, and the name "American Elk" on the map against "Wapiti" in the species list. Searching "elk" found nothing.
...
- **What:** I opened the live site minutes after the E-026 deploy and saw "species.json failed its integrity check (expected 19446acc…, got 5e653d26…)". The expected hash is the previous build's species.json, the received one is the new build's: an old app shell ran against new data.
...
- **What:** the tour summary was a pop-up that hid the map; I wanted it small enough that the map is always visible.
...
- **What:** a cleaner way to switch parks: a home page with a card per park, a tour that never blocks the map, the whole outline of the park first and then the zoom in, and proper transitions from one spot to the next.
...
- **What:** reading Grand Teton species pages, I noticed every model count was 0 and every busiest month was June or July, and asked why.
...
- **What:** the 320 px full-height dock was still too big; it covered the map and lost the sense of where everything was.
...
### E-033: making the camera pass count for something (my choice of 1 and 2, 2026-09-05)
- **What:** "model 0" on every species page was obviously not good. The options were: (1) a second, larger review to measure species-label precision by classifier score and move the naming bar if the numbers allow; (2) present the pass as what it is, a method with its own numbers, rather than a badge competing with human sightings; (3) a different sensor. Chosen: 1 and 2, "as long as it's free". Both are.
...
- **What:** my screenshot of the two-column tour card was four deploys old, and I was asking for a home page that already existed. My tab had never received a newer build.
...
- **Number:** my screenshot was build 4 of 8 that day.

### E-035: pictures from there, a closer camera, and motion during the tour
- **What:** with the corner card in place: the legend collided with the left panel once the planner opened; the tour's photographs should look like they were taken at the stop; the camera should be closer and the map should move while the tour runs.
...
- **What:** things to do at each main site: key features, where to camp, hikes, trails. And, mid-work: keep the UI clean and professional.
...
- **What:** I wanted the map to look like the place itself, a photo-realistic 3D view of what you are actually looking at. Measured what the free, keyless sources allow, then took what they give.
...
- **What:** the left panel should hide as well during the tour.
...
- **What:** trails and places needed details: click one and see which animals, how long the trail is, a summary and some history, pictures.
...
- **What:** after opening a park you should be able to see the whole map and click into another park from it, not only from the home page; and "includes model-predicted" in the legend looked unnecessary.
...
- **What:** when I started the tour and clicked a blue cell to see the animals, the motion stopped until I hit Next.
...
- **What:** the About page must not say a model is used where none actually ran.
- **Kept:** the page is now written per park. Where no camera pass ran it says so in one sentence and that every count comes from people; where one ran, the table. The Ask section says no language model is involved anywhere until Enable is pressed, and what the tour text, the numbers and the routes actually are. The 3D section says no models are used unless a species has one. Road and seasonal bias says "not measured here" outside Yellowstone. Two documents were added for the review I asked for: `docs/ARCHITECTURE.md` and `docs/USER-GUIDE.md`, linked from the About page and the README.

### E-046: the tour drives the road between stops
- **What:** on Zion: at a stop I want to be zoomed in, and Next should take me along the road to the next spot and zoom in there.
...
### E-047: the first drive lurched; the map now turns by hand
- **What:** after E-046 the drive did not work: the map got messed up on Next, and the map needed to spin 360 degrees rather than only zoom. The drive had not been watched before shipping (the browser tab it was checked in was hidden and ran no frame callbacks), so it had shipped on a state check alone.
- **Found:** two defects and one missing control. (1) `resample_v1` kept a per-segment carry and added it back into the running distance at every vertex, so its table said 21.5 km for a 16.9 km test road and spaced "25 m" samples anywhere from 5 to 50 m apart; the camera driven off that table crawled, jumped and, with the heading taken from the next sample, whipped around. Checked with a 400-vertex synthetic road: the rewrite's total equals the true length to 0.1 m and every gap is exactly the step. (2) The look-ahead point collapsed onto the end of the path in the last 120 m, so the heading swung toward north on arrival; the heading now comes from behind when nothing is ahead. (3) MapLibre rotates on a right-drag or two fingers, which nobody finds, and the tour's slow turn took the map back the instant a finger lifted, so it felt locked.
- **Kept:** the corrected resampler with a frame-rate-independent heading smoother; driver's height eased to zoom 14.8 and pitch 64° (72° in a canyon showed mostly sky and unloaded tiles); legs under 250 m fly; rotate-left/right and tilt buttons above the zoom control, on every map, and a legend line that names the gesture; the tour's turn now waits 10 s after any drag, wheel, pinch or button; a car/plane switch on the tour card chooses driving or flying between stops, remembered on the device.
- **Unresolved:** still unverified by eye; the next look at the live site is the test.

### E-048: the drive was underground; a headless probe that can see
- **What:** after E-047 the drive and the flight were still broken. The browser tab used for checks is hidden and runs no animation, so a probe was built instead: real Chrome headless with software WebGL, driven over the DevTools protocol from Node with no dependencies (`app/scripts/tour-probe.mjs`), sampling the camera every 200 ms and a screenshot every second.
- **Found:** on the live site, every frame from the first Next onward was blank white, and stayed white after landing. No WebGL loss, no MapLibre error, tiles loaded. The camera's target elevation was 0 m for the whole drive and after it, under ground that stands 1,200 to 1,700 m up: MapLibre's `jumpTo` looks the terrain up at the zoom it is handed, a fractional zoom finds no tile, and the lookup returns 0. MapLibre then keeps that height until the next camera move (`_elevationFreeze` stays set after a flight in 5.24), so the landing stayed underground and the screen showed fog. The flight alone landed right in the probe, but the same freeze means a flight that lands before its terrain tile arrives shows the same blank, which is what I saw. A second finding: the slow turn never ran. MapLibre fires `rotatestart` and `pitchstart` for its own camera moves, so the orbit's first nudge counted as a user touch and paused it for ten seconds, every time. A third: 46 km in 14 s at zoom 14.8 is unwatchable even when drawn (the heading swung 40° every half second).
- **Kept:** the ground height is passed to `jumpTo` explicitly from the last terrain tile that has it; an `idle` handler puts the target back on the ground after any landing that missed it; the pause responds only to events that carry a real input event. The drive now climbs with the leg: duration 3 s plus 1 s per 3.5 km within 4.5 to 16 s, and the cruise zoom is whatever makes the ground pass at 110 px/s (zoom 11.5 to 15.2, pitch 48° high to 64° low), looking 260 screen pixels up the road. Zion's 46 km leg cruises at zoom 12.2 with the valley in view and lands close; its 1.4 km leg to Angels Landing stays low at 15.2. Probe after the fix, two legs: target elevation follows the ground (1,996 → 1,228 → 2,398 m at Castle Dome, 1,974 m at Angels Landing), every frame drawn, bearing turning at rest.
- **Unresolved:** MapLibre 6.7 is out; whether it finalises the elevation after flights is unchecked. The first second of a climb can show unloaded tiles on a slow connection.

### E-049: where is this animal, park by park
- **What:** searching a species and clicking it should show where in each park that animal has been recorded most, instead of "Show on map" dropping me on whichever park was already selected.
- **Why it needed data work:** every park's files are separate and a park's cells run to a few MB, so answering across parks at query time would mean downloading all of them. `speciesindex.py` rolls the shipped park folders into one index at publish time: 735 species over 11 parks, per park the sightings, verified and model counts, cell count and the three busiest cells with centres; 561 KB, 120 KB gzipped, fetched on first need and hash-checked through parks.json like every other file. Genus- and family-level cell records (Laridae, Aves, Cetacea: 7,083 entries) are skipped, as species.json already skips them. Coarsened cells stay coarse; excluded species carry counts and no places.
...
- **What:** play and pause did not seem to work: after a click and a change of view the tour lost its motion and I had to press Next to get it back.
...
### E-051: the last 52 parks, and a data flow that leaves the working tree alone
- **What:** the remaining 52 parks, and a final clean-up. The parks: promoted from `config/parks.seed.toml` into `parks.toml` as they stand (iNaturalist place id and bbox; Indiana Dunes renamed to its 2019 title), no curated tours yet. The chain: `scripts/parks_batch.sh` runs Track A, landmarks, roads and things to do per park and hands every six to `scripts/publish_data.sh`, which copies the exports into a fresh git worktree on origin/main, rebuilds the species index and the park index there, and opens a data-only PR with auto-merge. Order: small parks first to prove the flow (Gateway Arch, Hot Springs, Cuyahoga Valley, Indiana Dunes), then by visitation, Alaska last. Logs in `data/batch/`. Started 2026-09-06 13:48.
- **Also in the first data PR (#39):** the eleven live parks re-exported after the overnight chain (camera pass on Moose Wilson, Cades Cove and Hayden Valley; things to do for all), and `sightings.parquet` dropped from the app folders: the app never read it and it was the largest file in every folder (O-8 resolved).
...
- **What:** why were there no 3D animals? Decided to remove the track, and asked for a pass over whether the project is good to share.
...
- **Left for me:** a LICENSE file for the code; the repository description, topics and homepage on GitHub; the species review CSV; running the Ask page's measurement on the live site.

### E-053: places, sorted by where people went, with when they go
- **What:** trails, hikes and sites sorted by how popular they are, the best times to see them, and pages for sites and monuments like the species pages.
- **The measure:** nobody publishes visitor counts per trail for free, so popularity is the sightings people recorded within reach of a place (500 m of a point, 300 m of a trail, open coordinates only): where observers went, the free proxy for where visitors go, said so on the page. Landmarks with a Wikipedia article also carry the article's average monthly readers from the Wikimedia pageviews API as an independent second sort. "Best time" is the month histogram of those sightings, shown as a sparkline with the busiest months named, with the caveat that it reflects when people looked.
- **Built:** `parkwild/places.py` rolls landmarks, named amenities and trails into `places.json` from the park's exported files (one in-memory DuckDB over the parquet; a bounding-box join then the exact distance; each sighting counted once per place), so it runs without the database and beside the batch. Unnamed items are dropped (forty rows called "Campsite" help nobody). Zion: 288 places, 96 KB; Yellowstone: 1,817 places, 600 KB, which is why the file loads on first open of the page. The Places page filters by kind, searches, sorts four ways and shows 150 rows before asking for a narrower query; the place page has the photograph and summary (from landmarks.json when present, live from Wikipedia and Commons otherwise), the month chart, the top species with photographs, and Show on the map and Add to a route. The batch chain gained the step and skips finished work on restart.
- **Checked:** headless on the built app: Zion's most recorded places are Riverside Walk (1,555, busiest May to July), West Rim Trail (1,478), Pa'rus Trail (1,295); Zion Lodge's page shows 739 sightings, 78 species, July, 360 readers a month, and Wild Turkey, Mule Deer, Black-headed Grosbeak, Western Bluebird, Rock Squirrel; Show on the map opens the trail on the map.

### E-054: a professional app layout, and the write side locked down
- **What:** professional formatting and app structure; the code protected while the repository stays public; proper security so the app is secure.
- **Structure:** `app/src` was one flat folder of twenty files. Now `src/app` (shell), `src/pages`, `src/components`, `src/lib` (pure logic with its tests beside it), `src/data` (loaders, park index, types), `src/store`, `src/styles`. Every import was rewritten by a script from a move table and checked by `tsc`; behaviour unchanged. ESLint (TypeScript recommended, `rules-of-hooks`, `exhaustive-deps`, unused code as an error) and Prettier at 120 columns run in the CI `app` job, now a required check on `main`. The linter found two escapes in the assistant's number check written oddly and three empty catch blocks with no reason; fixed. The first attempt at the move script tripped over a directory it had just created and my first escape "fix" landed on the wrong file; both are why the PR needed two follow-up commits.
- **Security:** reads stay public by design (a static site with no accounts and no write path), and the repository stays public so server-side protection is free. Added by API and by PR: secret scanning with push protection and non-provider patterns, Dependabot security updates and weekly update PRs (npm, pip, Actions), CodeQL default setup for TypeScript and Python, private vulnerability reporting with `/.well-known/security.txt` pointing at it, every GitHub Action pinned to a commit SHA, the workflow token read-only, and the `app` job added to the required checks. Already in place: protected `main` with strict checks, linear history, no force-push, no deletion, rules enforced for admins, `CODEOWNERS`, two-factor on the account, hashed data files, a page-level Content Security Policy.
- **Not done, and why:** signed commits are not required because I have no signing key registered yet; once one exists the rule is one call. GitHub Pages cannot send response headers, so `frame-ancestors` and the cross-origin headers in `_headers` wait for a host that reads that file (O-11, Cloudflare Pages, free).

### E-055: the fortnightly refresh, and the weather
- **What:** is the data fill part of the repository, can it run every couple of weeks, and can each location show its current weather?
- **Refresh:** the park batch was a one-off fill. `scripts/refresh.sh` now runs from cron on the 1st and 15th at 03:00: for every live park, `ingest --since <last run>` (iNaturalist `updated_since`, GBIF `lastInterpreted`, both added to the clients), dedupe, export, places, climate when older than a season, then one data PR through `publish_data.sh`. It refuses to start while the batch holds the database and never runs twice at once. A full re-pull of a park takes up to an hour; the incremental one takes minutes.
- **Weather, live:** current weather is a view-time fact, not a fortnightly one, so the browser asks Open-Meteo (free, keyless, CC BY 4.0) when a park or place is looked at: conditions now, wind, and the next three days, shown on the map panel at the park's busiest place, on every place page and in the place drawer. Fahrenheit first, Celsius in the tooltip. Nothing stored; a ten-minute cache per spot.
- **Weather, typical:** what can be precomputed is the climate. `climate.py` folds ten years of daily weather from the Open-Meteo archive (ERA5) into twelve monthly normals per park: typical high and low, rain, snow, wet days. Taken at the park's busiest place, not the bbox centre: Death Valley's centre is a mountain with a July high of 31 °C; Date Grove Picnic Area gives 44 °C, which is the Death Valley visitors meet. The archive answers 429 to a burst of ten-year requests; the fetch retries with backoff and the loop pauses between parks.
- **Checked:** Zion's panel headless: 65 °F rain, wind 6 mph, at Riverside Walk; Mon 82/60, Tue 84/60, Wed 91/70; typical for September 83/57 °F, 4 wet days. Two-park fixture tests for the normals and the file.

### E-056: how a change lands, written down
- **What:** the rules for landing a change lived in three places: SECURITY.md for the protected branch, docs/USER-GUIDE.md for one line on ship.sh, and my own memory notes for the rest (stage by name, one commit, a ledger entry, tagged constants, the voice, what I refuse). An unattended run has no memory notes, and CONTRIBUTING.md is the file anyone looks for, so I wrote one: the checks CI runs and how to run them locally, what the pre-push hook covers and what it does not, the branch, commit, pull request and auto-merge flow by hand, the notebook conventions, what I will not merge, and the five phases of `.claude/workflows/propose-review-ship.js`. Linked from the README's Running it yourself section in one line.
- **Kept:** one file of 75 lines, no new tooling or dependency. `ship.sh` is unchanged and described as it is: it branches from the current HEAD, commits everything with `git add -A`, then rebases onto origin/main. The overnight jobs write into `reports/` while I work, which is why PR #35 was the first change I staged by name and why E-051 moved the data publish into its own worktree; the by-hand flow is the one the file teaches. The after-merge step says `git pull --ff-only`, not `--rebase`: local main never carries its own commits, so a fast-forward always suffices and needs no stash, which is where I lost edits before.
- **Unresolved:** nothing ties the file to `ci.yml`, the Makefile, the hooks or the workflow script, and no test compares them; a check added, renamed or removed in any of those needs a line here by hand.
### E-057: the field-guide look, pictures that move, and the ground and rivers from USGS
- **What:** I wanted a premium look rather than a flat white page, thumbnails that feel like moving pictures, and USGS data folded into what exists.
- **Look:** design system v2. A dark field-guide default: deep green-charcoal surfaces with a warm ivory type, one blue accent kept for the map's cells, depth from two soft radial glows and a faint grain over the ground, glass panels with a hairline highlight. Headings in a self-hosted display serif (Instrument Serif, OFL, 15 KB per face, latin subset), so no request leaves for a font host. The light set stays behind `data-theme="light"`. Same scales as v1, so nothing moved.
- **Motion:** every thumbnail and hero drifts slowly (a 28 s scale and pan, staggered by grid position in CSS so a grid never moves in lockstep), pauses and zooms under the pointer, and the home hero photograph drifts behind the words. All of it off under reduced motion.
