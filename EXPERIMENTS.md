...
- **What:** the owner, after E-046: "I don't think it's working, the map and everything gets all messed up, and can we have the map so we can spin around 360 degrees, not just zoom in and out". The drive could not be watched from here (the automation tab is hidden and frame callbacks do not run), so it shipped on a state check alone.
...
- **What:** the owner: "when I search for species of animals and I click on them I want to be able to have the option to see where in each of the parks that animal has appeared the most, instead of clicking on a species and Show on map taking me to an already selected map".
...
- **What:** the owner: "trails and hikes and sites sorted by most popular, best times to see them, and pages for sites and monuments like the species pages".
...
- **What:** the owner: "professional formatting and app structure; the code protected but the repository public; good security methods so the app is secure".
...
- **Not done, and why:** signed commits are not required because the owner has no signing key registered; once one exists the rule is one call. GitHub Pages cannot send response headers, so `frame-ancestors` and the cross-origin headers in `_headers` wait for a host that reads that file (O-11, Cloudflare Pages, free).

### E-055: the fortnightly refresh, and the weather
- **What:** the owner: "is the autofill process included, and can we have it run every couple of weeks with the current weather at each location?"
